...
@Controller
class BalancerServlet extends HttpServlet implements Servlet {
...
//		configRoot = config.getInitParameter("root");
		...
		configRoot = System.getProperty("root");
		bldConfig(configFile);
		ThreadGroup loadBalancerThreads = new ThreadGroup(LoadBalancerStatic.THREADGROUP);
		new Thread(loadBalancerThreads,new InstancesHealthCheck(),"BalancerServlet").start();
	...
	private void processFile(HttpServletRequest request, HttpServletResponse response) {
		...
		if (reqURL.endsWith("js")) type = "text/javascript";
		...
		try {
			String htmlName = request.getPathInfo();
			if (htmlName == null) htmlName = "/index.xhtml";
			if (htmlName.length() > 0) htmlName = htmlName.substring(1);
			InputStream is = LoadBalancerStatic.htmlFiles.get(htmlName);
			if (is == null) return;
	...
	private void performTask(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getContextPath().length() > 0)
			application = request.getContextPath().substring(1);
		...
		String url = service+"?application="+application+"&loadBalancerWSURL="+String.format("ws://%s:%d%s/websocketserver", host,port,balancer); 
